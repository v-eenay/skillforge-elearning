        ...
        try {
            ...
            switch (contentBlock.getBlockType()) {
                case TEXT:
                    ...
                    break;
                case VIDEO:
                    content = request.getParameter("videoUrl");
                    description = request.getParameter("videoDescription");
                    if (content == null || content.trim().isEmpty()) {
                        request.setAttribute("error", "Video URL is required for VIDEO blocks.");
                        doGet(request, response);
                        return;
                    }
                    break;
                case IMAGE:
                    content = request.getParameter("imageUrl");
                    description = request.getParameter("imageCaption");
                    if (content == null || content.trim().isEmpty()) {
                        request.setAttribute("error", "Image URL is required for IMAGE blocks.");
                        doGet(request, response);
                        return;
                    }
                    break;
                case CODE:
                    ...
                    break;
                case DOCUMENT:
                    content = request.getParameter("documentUrl");
                    description = request.getParameter("documentDescription");
                    if (content == null || content.trim().isEmpty()) {
                        request.setAttribute("error", "Document URL is required for DOCUMENT blocks.");
                        doGet(request, response);
                        return;
                    }
                    break;
